...
<style>
  ...
  .new-chat-btn{ padding:8px 12px; background: linear-gradient(90deg,var(--accent),#00aaff); color:#fff; border:none; border-radius:10px; font-weight:800; cursor:pointer; box-shadow: 0 10px 26px rgba(0,120,255,0.12); }
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      function sendAttachmentNow(){
        try{
          ...
          setTimeout(()=>{
            typingEl.remove();
            const reply = simulatedAssistantReply(text || ('Sent ' + (userMessage.attachment.name || userMessage.attachment.type)));
            const assistantMessage = {id:genId(), role:'assistant', content:reply, parent: messages.length - 1, meta:{}};
            messages.push(assistantMessage);
            appendMessage(assistantMessage, messages.length - 1);
            saveSession();
          }, 700 + Math.min(1500, (text || '').length*18));
      ...
      function simulatedAssistantReply(prompt){
        prompt = (prompt||'').trim();
        if(!prompt) return "Could you provide a little more detail on what you want?";
        const low = prompt.toLowerCase();
        if(low.includes('summarize')) return "Here's a concise summary: ...";
        if(low.includes('quiz')) return "Short quiz:\n1) ...\n2) ...\n3) ...";
        if(low.includes('explain')) return "I'll explain step-by-step: ...";
        if(low.includes('plan')) return "Two-week study plan: Week 1 ... Week 2 ...";
        return "Focused answer:\n• Key idea: " + prompt.split('.').slice(0,1).join('');
      }

      // Typing indicator — do not force scroll if user is scrolled up (autoScrollEnabled will control)
      function showTyping(){ 
        const t = document.createElement('div'); 
        t.className = 'typing msg assistant show'; 
        t.innerHTML = `<div class="dots"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div>`; 
      ...
      function sendMessage(text){
        ...
        setTimeout(()=>{
          typingEl.remove();
          const reply = simulatedAssistantReply(text);
          const userIndex = messages.length - 1;
          
          const assistantMessage = {id:genId(), role:'assistant', content:reply, parent:userIndex, meta:{}};
          messages.push(assistantMessage);
          appendMessage(assistantMessage, messages.length - 1);
          saveSession();
        }, 700 + Math.min(1500, text.length*18));
      ...
      sendInsideBtn.addEventListener('click', (e)=>{
        ...
        setTimeout(()=>{
          ...
          if(editIndex !== null && typeof editIndex === 'number'){
            ...
            for(let i = messages.length-1; i>editIndex; i--){
              if(messages[i].role === 'assistant' && messages[i].parent === editIndex){
                messages.splice(i,1);
              }
            }
            const newReply = simulatedAssistantReply(t);
            messages.splice(editIndex+1, 0, { id: genId(), role:'assistant', content: newReply, parent: editIndex, meta: {} });
            saveSession();
            renderMessages();
            toast('Message updated and resent');
            editIndex = null;
      ...
      messagesEl.addEventListener('click', async (e)=>{
        ...
        if(act === 'copy'){
          ...
          setTimeout(()=>{
            typingEl.remove();
            msg.content = simulatedAssistantReply(userText);
            msg.meta = {};
            saveSession();
            updateThisMessage();
            el.disabled = false;
          ...
          if(msg.meta.tempDisliked){
            ...
            msg.meta.tempDisliked = true;
            msg.meta.awaitingFeedback = true;
          }
          saveSession();
          updateThisMessage();
        }
          ...
          msg.meta.tempDisliked = false;
          msg.meta.hideDislike = false;
          msg.meta.locked = true;
          saveSession();
          updateThisMessage();
          toast('Thank you for your report');
          const logs = JSON.parse(localStorage.getItem(REPORTS_KEY) || '[]');
          logs.push({ type:'dislike', messageId: msg.id, text: val, at: Date.now() });
      ...
      messageConfirmModal.querySelector('[data-msg-confirm="yes"]').addEventListener('click', ()=>{ 
        if(typeof pendingMessageDeleteIndex === 'number'){
          messages.splice(pendingMessageDeleteIndex);
    ...
    ${rows}
      ...
      function exportConversation(){
        try {
          ...
          const title = (data && data.length>0) ? (data.find(m=>m.role==='user' && m.content && m.content.trim()) ? data.find(m=>m.role==='user' && m.content && m.content.trim()).content.split(/\s+/).slice(0,4).join(' ') : 'Conversation') : 'Conversation';
      ...
      reportModal.querySelector('[data-report="send"]').addEventListener('click', ()=>{
        const payload = {
      ...
      function generateConversationTitle(msgs){
        try{
          ...
          const user = msgs.find(m => m.role==='user' && m.content && m.content.trim());
          if(user){
...
def handle_report_function(report_json_string):
    try:
        if not report_json_string:
            print("تم استلام طلب فارغ، تم التجاهل.")
